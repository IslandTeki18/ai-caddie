...
import { Screen, CenteredScreen, Card, PrimaryButton } from '@/ui/components';
    ...
    <Screen>
      ...
      {plays ? (
        <View className="mb-6">
          ...
          {expanded ? (
            <View className="mt-2 rounded-xl bg-surface-2 p-3">
...
function Row(props: { label: string; value: string; big?: boolean }) {
  return (
    <View className="mb-2">
      ...
      <Text className={props.big ? 'text-3xl font-bold text-accent' : 'text-base text-fg'}>
        {props.value}
      </Text>
    </View>
  );
...
function PlayRow(props: { label: string; v: number }) {
  ...
  return (
    <View className="flex-row justify-between">
      <Text className="text-fg-muted">{props.label}</Text>
      <Text className="text-fg">{sign}</Text>
    </View>
  );
}
